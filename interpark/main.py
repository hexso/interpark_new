...
class Form(QWidget):
    ...
    def start(self):
        ...
        for i in range(self.sb_program_cnt.value()):  # ✅ 각 검색어를 별도 프로세스로 실행
            ...
            interpark_file_path = os.path.join(current_dir, "interpark.py")
            credential_file_path = os.path.join(current_dir, f'id{i}.txt')

            process.setArguments([interpark_file_path, str(inter_ticket_id), credential_file_path])  # ✅ 인덱스 값과 검색어 전달
